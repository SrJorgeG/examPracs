...
static long parse_expr(char **expr);
static long parse_term(char **expr);
static long parse_factor(char **expr);
...
static long parse_term(char **expr)
{
	...
	val = parse_factor(expr);
	while (**expr == '*')
	{
		(*expr)++;
		rhs = parse_factor(expr);
		val *= rhs;
	}
	return (val);
}
